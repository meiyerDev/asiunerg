...
export default {
  ...
  methods: {
    ...
    newStudent(form) {
      this.infoToast({ message: "Registrando estudiante." });
      axios
        .post("/admin/estudiantes", form)
        .then(response => {
    ...
    updateStudent(form) {
      ...
      axios
        .put(`/admin/estudiantes/${form.id}`, form)
        .then(response => {
          if (response.status === 200) {
    ...
    postExcel(e) {
      ...
      this.loadExcel = true;
      this.infoToast({ message: "Subiendo Archivo." });
      axios
        .post("/admin/estudiantes/nuevo/excel", formData)
        .then(response => {
          this.getStudents();
          this.successToast({ message: `${response.data.message}` });
        })
        .catch(error => {
          if (error.response.status === 422) {
            ...
            for (const e in error.response.data.errors) {
              if (error.response.data.errors.hasOwnProperty(e)) {
                const element = error.response.data.errors[e];
                this.errorToast({ message: element });
              }
            }
          } else if (error.response.status === 400) {
          ...
          <div class="row align-items-center">
            ...
            <div class="col-8 col-lg-4 text-right">
              <button
                ...
                :disabled="loadExcel"
              >{{ !loadExcel ? 'Cargar Excel' : 'Cargando..'}}</button>
            ...
            <tbody>
              <tr v-for="student in students" :key="student.id">
                ...
                <td class="text-center">
                  <button
                    class="btn btn-sm btn-info btn-icon-only"
                    @click.prevent="getStudent(student.id, 'matter')"
                  >
                    <i class="fas fa-eye"></i>
                  </button>
                  <button
                    class="btn btn-sm btn-warning btn-icon-only"
                    @click.prevent="getStudent(student.id, 'edit')"
                  >
